...
- `requireAuth` revalidates every session user against the database. The role lookup uses a per-process cache (`AUTH_USER_CACHE_TTL_MS`, default 15s), while `session_version` is checked directly on every authenticated request so password reset/setup revokes old cookies immediately across instances. Login stores the current version in the signed session; stale versions receive 401. New unit tests cover version mismatch in addition to cache hit/miss, role demotion, deleted-user 401, negative-cache TTL, and DB error forwarding.
...
- Self-service account setup (Issue #62): `POST /users` (team_lead only) creates a member with `passwordHash: null`, a one-time setup token (sha256-hashed in the DB, TTL `SETUP_TOKEN_TTL_MS` default 24h), and returns `{ user, setupToken, expiresAt }`. `POST /auth/setup-account` (public) atomically claims the token, hashes the password, increments `session_version`, clears the token columns, starts a session, and returns the user. `POST /users/:id/reset-password` (team-lead only, no self-reset, no resetting the only team_lead) increments `session_version` and issues a fresh token, revoking existing sessions. `Role` enum is widened to `member | team_lead | admin` (`admin` is a forward-compatible role used by `canManageUsers` / `canModifyPto`; today the team_lead gate is the active one). The `Prisma User` table gets nullable `password_hash`, `setup_token_hash VARCHAR(64)`, `setup_token_expires_at`, and `session_version INTEGER NOT NULL DEFAULT 0`. New setup links use `/setup-account#token=...`; legacy query-token links are scrubbed on load. The frontend exposes `/setup-account#token=...` (`SetupAccountPage`) and a team-lead-only `/admin/users` page (`AdminUsersPage` with a list + "Add user" form + "Reset password" buttons). The new `Role.admin` enum value is in the Prisma schema and 4 TS files (cookieSession, AuthorizationService, frontend permissions, frontend types/api) plus `docs/openapi.yaml` and `docs/schema.sql` (per the issue's implementation-gaps list). `docs/openapi.yaml` adds the `Password`, `SetupAccountRequest`, `CreateUserRequest`, `CreateUserResponse`, `ResetPasswordResponse` schemas and `POST /auth/setup-account`, `GET /users`, `POST /users`, `POST /users/{id}/reset-password` paths with FORBIDDEN 403 documentation. Unit tests in `backend/src/services/users/UserService.test.ts` cover every branch (14 cases). Integration tests in `backend/tests/integration/users.test.ts` cover the routes end-to-end (17 cases, including session revocation and concurrent token redemption). Frontend unit tests in `SetupAccountPage.test.tsx` (7 cases) and `AdminUsersPage.test.tsx` (2) cover the new pages. The setup-account route is rate-limited by the existing login limiter. Audit-log entries are written for `create_user` and `reset_user_password` actions.
