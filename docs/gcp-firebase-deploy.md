# GCP/Firebase Fallback Deployment & Operations

This runbook preserves the previous GCP/Firebase production path. OCI is the
primary production deployment; use these workflows only after deliberately
enabling the GCP fallback path.
...
Set repository variables when the GCP fallback is intentionally enabled:

| Variable | Example |
|---|---|
| `GCP_FALLBACK_DEPLOY_ENABLED` | `true` |
...
The four GCP workflows are manual-only. The main `deploy.yml` workflow checks
`GCP_FALLBACK_DEPLOY_ENABLED`, validates the tested commit, builds the image,
applies migrations, deploys Cloud Run, and publishes Firebase Hosting. Keep
the protected `production` environment and required reviewers enabled.
...
|---|---|---|---|---|---|
| `deploy.yml` | `workflow_dispatch` | builds + tags with the commit SHA | builds + deploys | yes | yes |
...
The OCI workflow owns tag-triggered releases. The GCP workflows stay
`workflow_dispatch` only so a release tag cannot deploy to both platforms.

### Running the GCP fallback
...
1. The workflow checks out `master` and validates the successful CI run for
   that commit.
2. After the gate passes, it builds and pushes the image, applies Prisma
   migrations, rolls out Cloud Run, publishes Firebase Hosting, and runs the
   cross-origin smoke test.

Do not push the OCI release tag while `GCP_FALLBACK_DEPLOY_ENABLED=true`.

### Picking the right workflow

| Action | Workflow |
|---|---|
| Running the GCP fallback | `deploy.yml` (manual, from `master`) |
...
The fallback workflow's Docker image is tagged with the commit SHA. The tag
based image example below applies only to the historical GCP release flow:
...
- **Secret rotation only** (the canonical case from the recent
...
production cutover + DR drill checklist in
  [`gcp-firebase-cutover-drill.md`](gcp-firebase-cutover-drill.md). It exercises every step
