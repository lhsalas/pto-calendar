# Production Cutover & Disaster Recovery Drill

This runbook validates the end-to-end production deployment (Cloud Run API +
Firebase Hosting SPA + Supabase Postgres) before the OCI / Deno production
infrastructure is decommissioned. It is split into two halves:
...
- Google Cloud, Firebase, and Supabase projects configured per
  [`docs/deploy.md`](deploy.md) § 1–§ 4.
- GitHub repository variables / secrets set per
  [`docs/deploy.md`](deploy.md) § 4.
...
`Deploy production` workflow builds `backend/Dockerfile`, runs the
...
bootstrap command from [`docs/deploy.md`](deploy.md) § 7:
...
**not** complete — file a follow-up issue before merging the
decommission of the OCI / Deno infrastructure (issue #129).
