...
- `frontend/src/api/client.ts` `apiRequest` enforces a default 15s per-request timeout (configurable via `VITE_API_TIMEOUT_MS` or the `timeoutMs` option), normalizes fetch errors into a typed `ApiError(0, { code: 'TIMEOUT' | 'ABORTED' | 'NETWORK' | 'BAD_RESPONSE', message })`, and accepts a caller `signal` to abort in-flight requests. A `try/catch` around `JSON.parse(text)` throws `BAD_RESPONSE` for malformed (non-JSON) bodies so the caller never gets an uncaught `SyntaxError` from the fetch layer
