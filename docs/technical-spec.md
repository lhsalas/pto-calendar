...
- HTTP-only secure cookie sessions (`cookie-session`). `COOKIE_SAME_SITE` defaults to `lax` locally and is set to `none` for the default split Firebase Hosting + Cloud Run deployment; `COOKIE_SECURE=true` is required whenever `none` is used
- Cross-origin requests are allowed via the `cors` middleware using the `CORS_ORIGIN` env var allowlist. Credentials are permitted (`credentials: true`) so the HTTP-only cookie session works across origins in production. State-changing requests also pass through `csrfOriginMiddleware`, which rejects a supplied `Origin` or `Referer` outside the configured origin
...
- Production deployment uses Firebase Hosting for the static SPA, Cloud Run for the Node API, and Supabase for PostgreSQL. Firebase Hosting does not rewrite API requests to Cloud Run because its CDN strips incoming cookies except for `__session`; the Vite client calls the Cloud Run origin directly. `CORS_ORIGIN` is the exact Firebase origin, `COOKIE_SAME_SITE=none`, `COOKIE_SECURE=true`, and `TRUST_PROXY_HOPS=1` are used for the split-origin deployment. Firebase Hosting emits the static CSP and security headers from `firebase.json`; Cloud Run emits API headers through `helmet`. `min-instances=0` is explicit and a small maximum instance limit protects Supabase connection capacity
...
- Back up the database using the external Supabase CLI/GCS workflow described in `docs/database-backups.md`
- Configure environment variables for DB and auth secrets through Cloud Run Secret Manager references
- Set `CORS_ORIGIN` to the Firebase production origin. For the default split-host deployment, use `COOKIE_SECURE=true`, `COOKIE_SAME_SITE=none`, and an empty `COOKIE_DOMAIN`
